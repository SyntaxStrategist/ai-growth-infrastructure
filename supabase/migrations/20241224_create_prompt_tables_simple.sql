-- Simple migration to create prompt optimization tables
-- This avoids the complex functions that might be causing issues
...
-- Add comments
COMMENT ON TABLE public.prompt_performance IS 'Tracks individual prompt execution results';
COMMENT ON TABLE public.prompt_ab_tests IS 'Manages A/B testing of prompt variants';
COMMENT ON TABLE public.prompt_evolution IS 'Tracks prompt evolution history';

-- Success message
DO $$
BEGIN
  RAISE NOTICE '✅ Prompt optimization tables created successfully!';
END $$;
